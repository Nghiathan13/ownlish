...
describe("dictation progress API", () => {
  ...
  it("submits the raw typed answer and returns progress", async () => {
    apiRequest.mockResolvedValue(progress);
    await expect(submitDictationAnswer("token", { videoId: "video-1", segmentId: "s001", answer: "Hello world" })).resolves.toEqual(progress);
    expect(apiRequest).toHaveBeenCalledWith("/dictation/videos/video-1/answers", {
      method: "POST", token: "token", body: JSON.stringify({ segmentId: "s001", answer: "Hello world" }),
    });
  });
  ...
  it("resets progress and rejects an invalid answer response", async () => {
    ...
    await expect(submitDictationAnswer("token", { videoId: "video-1", segmentId: "s001", answer: "Hello world" })).rejects.toThrow("invalid");
