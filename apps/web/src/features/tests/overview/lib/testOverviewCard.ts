import { iconTextButtonClassName } from "@/shared/ui/button";

export const testOverviewCardClassName =
  "flex min-w-0 flex-col gap-4 rounded-[16px] bg-surface p-4 shadow-card";

export const testOverviewCardGridClassName =
  "grid gap-8 sm:grid-cols-2 xl:grid-cols-4";

export const testOverviewCardSkeletonClassName = "min-h-40 w-full rounded-[20px]";

export const testOverviewMockButtonClassName = iconTextButtonClassName(
  "min-w-0 flex-1 shrink border-border bg-transparent text-foreground hover:bg-hover-overlay",
);

export const testOverviewPracticeButtonClassName = iconTextButtonClassName(
  "min-w-0 flex-1 shrink border-foreground bg-foreground text-background hover:[box-shadow:inset_0_0_0_9999px_var(--hover-overlay-solid)]",
);
