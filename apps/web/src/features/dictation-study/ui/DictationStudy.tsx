...
type DictationStudyProps = {
  backHref: string;
  catalogVideos: DictationCatalogVideo[];
  initialProgress: DictationProgress | null;
...
export function DictationStudy({
  backHref,
  catalogVideos,
  initialProgress,
  video,
  ...
  const [answeredSegmentIds, setAnsweredSegmentIds] = useState<string[]>(
    () => initialProgress?.answeredSegmentIds ?? [],
  );
  ...
  useEffect(() => {
    ...
    void runAuthenticatedRequest({
      ...
      .then((progress) => {
        setDictationProgressQueryData(queryClient, user?.id ?? null, progress);
      })
      .catch(() => {
        setAnsweredSegmentIds((current) =>
