...
import { DashboardProgressPage } from "./DashboardProgressPage";
...
vi.mock("@/features/dashboard-progress", () => ({
  DashboardProgressPanel: () => <div>Progress panel</div>,
}));
...
describe("DashboardProgressPage", () => {
  beforeEach(() => {
  ...
  it("marks Progress active and composes the progress panel", () => {
    ...
    expect(screen.getByText("Progress panel")).toBeInTheDocument();
    expect(screen.queryByText("Activity panel")).not.toBeInTheDocument();
  });

  it("shows the progress skeleton while collections load", () => {
    mocks.useCollectionsListQuery.mockReturnValue({
      collections: [],
      collectionsError: null,
